...
class RockPaperScissorsGame {
    ...
    displayResult(playerChoice, computerChoice, result) {
        if (this.playerChoiceEl) {
            this.playerChoiceEl.textContent = `${this.choiceEmojis[playerChoice]} ${playerChoice.toUpperCase()}`;
        }
        
        if (this.computerChoiceEl) {
            this.computerChoiceEl.textContent = `${this.choiceEmojis[computerChoice]} ${computerChoice.toUpperCase()}`;
        }
    ...
    updateDisplay() {
        ...
        if (this.winRateEl && this.gameState.totalGames > 0) {
            const winRate = ((this.gameState.playerScore / this.gameState.totalGames) * 100).toFixed(1);
    ...
    addToHistory(playerChoice, computerChoice, result) {
        ...
        historyItem.className = 'flex justify-between items-center p-2 bg-gray-100 dark:bg-gray-700 rounded mb-2';
    ...
    resetGame() {
        ...
        if (this.gameResultEl) {
            ...
            this.gameResultEl.className = 'text-xl font-bold text-gray-600 dark:text-gray-300';
        }
