...
class MobileNavigation {
    ...
    initializeElements() {
        ...
        if (this.elements.mobileMenuButton) {
            this.elements.hamburgerLines = this.elements.mobileMenuButton.querySelectorAll('span');
        }
        
        // Get mobile menu links if menu exists
        if (this.elements.mobileMenu) {
            this.elements.mobileMenuLinks = this.elements.mobileMenu.querySelectorAll('a');
        }
        
        // Validate required elements
        if (!this.elements.mobileMenuButton || !this.elements.mobileMenu) {
            Utils.debug('Mobile navigation elements not found - mobile navigation disabled');
            return false;
        }
    ...
    initializeAccessibility() {
        if (this.elements.mobileMenuButton) {
            this.elements.mobileMenuButton.setAttribute(CONSTANTS.ARIA.EXPANDED, 'false');
        }
    }
    ...
    openMobileMenu() {
        ...
        this.elements.mobileMenuButton.setAttribute(CONSTANTS.ARIA.EXPANDED, 'true');
        
        // Focus management for accessibility
        if (this.elements.mobileMenuLinks && this.elements.mobileMenuLinks.length > 0) {
     ...
     */
    animateHamburgerToX() {
        if (!this.elements.hamburgerLines || this.elements.hamburgerLines.length < 3) {
            return;
        }
